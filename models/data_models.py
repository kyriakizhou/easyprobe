...
class LayerOption(Enum):
    """Which layers to extract activations from."""

    ALL = "all"  # All layers in the model
...
class AggregationMethod(str, Enum):
    """Method to aggregate scores across tokens."""
    MAX = "max"
    MEAN = "mean"
    FIRST = "first"
    LAST = "last"
...
LayerSpec = Union[LayerOption, str, list[int], range]  # LayerOption.ALL, "all", [0, 5, 10], or range(0, 32)
PositionSpec = Union[PositionOption, int, list[int]]
ComponentSpec = Optional[list[ComponentOption]]
ActivationKey = tuple[int, ComponentOption] # (layer_index, component_option)
...
class SingleFeatureData:
    ...
    def get_dataset_info_string(self) -> str:
        """Returns a formatted string describing the dataset size and label distribution."""
        positive_count = sum(self.labels)
        return f"Dataset: {self.num_samples} prompts ({positive_count} positive, {self.num_samples - positive_count} negative)"
...
class MultiFeatureSharedPromptsData:
    ...
    def get_dataset_info_string(self) -> str:
        """Returns a formatted string describing the dataset size and feature distribution."""
        lines = [f"Probing {self.num_features} features on {self.num_samples} shared prompts", f"  Features: {', '.join(self.feature_names)}"]
        for feature_name, labels in self.features.items():
            positive_count = sum(labels)
            lines.append(f"  {feature_name.capitalize()}: {positive_count} positive, {self.num_samples - positive_count} negative")
        return "\n".join(lines)
...
class MultiFeatureSeparatePromptsData:
    ...
    def get_dataset_info_string(self) -> str:
        """Returns a formatted string describing the dataset size and feature distribution."""
        lines = [f"Probing {self.num_features} features with separate prompts:"]
        for feature_name, (prompts, labels) in self.features.items():
            positive_count = sum(labels)
            lines.append(f"  {feature_name}: {len(prompts)} prompts ({positive_count} positive, {len(labels) - positive_count} negative)")
        return "\n".join(lines)
...
class ProbeTask:
    ...
    norm_mean: Optional[np.ndarray] = None  # Z-score mean from training normalization, shape: (hidden_dim,)
    norm_std: Optional[np.ndarray] = None  # Z-score std from training normalization, shape: (hidden_dim,)
